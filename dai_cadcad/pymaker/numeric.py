...
class Wad:
    ...
    def __init__(self, value):
        ...
        if isinstance(value, Wad):
            ...
            self.value = int(
                (Decimal(value.value) // (Decimal(10) ** Decimal(9))).quantize(
                    1, context=_context
                )
            )
        elif isinstance(value, Rad):
            self.value = int(
                (Decimal(value.value) // (Decimal(10) ** Decimal(27))).quantize(
                    1, context=_context
                )
            )
    ...
    def __str__(self):
        ...
        return (tmp[0 : len(tmp) - 18] + "." + tmp[len(tmp) - 18 : len(tmp)]).replace(
            "-.", "-0."
        )
    ...
    def __mul__(self, other):
        if isinstance(other, Wad):
            ...
            result = (
                Decimal(self.value)
                * Decimal(other.value)
                / (Decimal(10) ** Decimal(45))
            )
            return Wad(int(result.quantize(1, context=_context)))
        elif isinstance(other, int):
            return Wad(
                int(
                    (Decimal(self.value) * Decimal(other)).quantize(1, context=_context)
                )
            )
        else:
            raise ArithmeticError
...
class Ray:
    ...
    def __init__(self, value):
        ...
        if isinstance(value, Ray):
            ...
            self.value = int(
                (Decimal(value.value) / (Decimal(10) ** Decimal(18))).quantize(
                    1, context=_context
                )
            )
        elif isinstance(value, int):
    ...
    def __str__(self):
        ...
        return (tmp[0 : len(tmp) - 27] + "." + tmp[len(tmp) - 27 : len(tmp)]).replace(
            "-.", "-0."
        )
...
class Rad:
    ...
    def __str__(self):
        ...
        return (tmp[0 : len(tmp) - 45] + "." + tmp[len(tmp) - 45 : len(tmp)]).replace(
            "-.", "-0."
        )
