...
def run_test():
    ...
    assert round(util.wad_to_float(sample_urn["art"]), 12) == round(
        util.wad_to_float(eth_price * 4 / 7), 12
    ...
    assert round(util.rad_to_float(sample_dai), 12) == round(
        util.rad_to_float(eth_price * 4 / 7 * vat["ilks"]["eth"]["rate"]), 12
